...
descr(c_m) %>% tab_df(title = "Estadistica del cancer de mama y su nivel de expresion de los diferentes genes",
file = "mRNA_expr_summary.doc") 
```

### [0.5 PUNTOS] 1.6 Interprete los resultados de la tabla resultante de la sección 1.5. 
```{tex}
# The table is a summary of data and it can be seen that the genes with the highest gene expression are ESR1 and MUC1, unlike XBP1 and GATA3, which are expressed to a lesser extent.
```
...
graficasc <- reg_gen_expression("GATA3")
graficasc[[5]][[2]]
...
```{tex}
# There is a gene overexpression of the LUSC gene, since it is higher than average.
```
